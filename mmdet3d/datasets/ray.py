...
def get_rays(H, W, K, c2w, inverse_y=True):
    i, j = torch.meshgrid(
...
def pts2ray(coor, label_depth, label_seg, c2w, cam_intrinsic, width, height):
    rays_o, rays_d, viewdirs = get_rays(W=width, H=height, K=cam_intrinsic,c2w=c2w)
    coor = coor.long()
    x,y = coor[:,0], coor[:,1]
...
def generate_rays(coors, label_depths, label_segs, c2w, intrins, width, height, max_ray_nums=0, time_ids=None, dynamic_class=None, balance_weight=None, weight_adj=0.3, weight_dyn=0.0, use_wrs=True):
    r"""NuScenes Dataset.
    ...
    for time_id in time_ids:    # multi frames
        for i in time_ids[time_id]: # multi cameras of single frame
            ray = pts2ray(coors[i], label_depths[i], label_segs[i], c2w[i], intrins[i], width, height)
            rays.append(ray)
            ids.append(time_id)
